...
for csv_path in score_csv_list:
    ...
    top_list.append(
        tmp.sort_values(by="dev_hauc", ascending=False).iloc[:n_top, [0, 1, 4]]
    )
top_df = (
    pd.concat(top_list, axis=0)
    .sort_values(by="dev_hauc", ascending=False)
...
for machine in machines:
    ...
    print(
        f"hauc:{score:.4f}, std:{score_std:.4f} n_mix:{n_mix} {machine} n_top:{n_top}"
    )
...
for machine in machines:
    ...
    save_df[f"{machine}_normal_score"] = (
        seed_df_dict[machine].iloc[:, 5 : n_mix + 5].values.mean(1)
    )
